...
function Get-ModuleName {
  return "Install .NET SDK 8"
}

function Test-Execute {
  $dotnetPath = Get-Command "dotnet" -ErrorAction SilentlyContinue
  if (-not $dotnetPath) {
    Write-Host ".NET SDK 8 is not installed"
    return $true
  }

  $dotnetSdks = & "dotnet" --list-sdks -ErrorAction SilentlyContinue
  if ($dotnetSdks -and $dotnetSdks -match "8\.0\.\d+") {
...
function Execute {
  $chocoCommand = "choco install dotnet-8.0-sdk -y -f"
  ...
  Sync-ChocoEnv
  Write-Line ".NET SDK 8 installation succeeded"
}
